...
def fbs_system(prior_summary=None):
    ...
    return (
        ...
        "suggestion. This is a negotiation, not a one-shot approval.\n\n"
        "IMPORTANT — closing a layer is the designer's action, not yours: once "
        "the designer accepts a candidate, restate its final label clearly and "
        "then STOP. Do not start proposing the next layer's candidates in that "
        "same reply, even if the path forward seems obvious. The interface only "
        "advances when the designer clicks a 'Close layer' button on their end, "
        "and they'll come back to you afterward (e.g. 'ok, propose Behaviour "
        "now') to open the next layer. Jumping ahead just creates a mismatch "
        "between what you've said and what the interface actually shows as "
        "closed.\n\n"
...
def opening_prompt(req):
    ...
    return (
        ...
        "For each layer: propose, negotiate, and once agreed, state its final "
        "label clearly and STOP there — wait for the designer to close it on "
        "their end and explicitly ask you to open the next layer. Don't move to "
        "the next layer's candidates on your own.\n"
        ...
        "then stop once Function is agreed — the designer closes each layer on "
        "their end and will prompt you when it's time for the next one."
    )
